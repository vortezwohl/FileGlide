...
def test_file_create_and_exists(cli, runner, workspace: Path) -> None:
    result = runner.invoke(cli, ["file", "create", "--root", str(workspace), "sample.txt"])
    assert result.exit_code == 0, result.output
    payload = parse_json(result.output)
    assert payload["ok"] is True
    assert payload["result"]["entry"]["name"] == "sample.txt"

    exists = runner.invoke(cli, ["file", "exists", "--root", str(workspace), "sample.txt"])
...
def test_path_delete_dry_run_returns_preview(cli, runner, workspace: Path) -> None:
    ...
    result = runner.invoke(
        cli,
        ["path", "delete", "--root", str(workspace), "--recursive", "--dry-run", "to-delete"],
    )
    assert result.exit_code == 0, result.output
    payload = parse_json(result.output)
    assert payload["ok"] is True
...
def test_text_read_write_and_replace_lines(cli, runner, workspace: Path) -> None:
    ...
    read_result = runner.invoke(
        cli,
        ["text", "read", "--root", str(workspace), "--start-line", "2", "--end-line", "3", "editable.txt"],
    )
    assert read_result.exit_code == 0, read_result.output
    read_payload = parse_json(read_result.output)
    assert [line["text"] for line in read_payload["result"]["lines"]] == ["second", "third"]
    ...
    assert (workspace / "editable.txt").read_text(encoding="utf-8") == "first\nupdated\nthird\n"



def test_regex_search_and_fuzzy_search(cli, runner, workspace: Path) -> None:
    grep_result = runner.invoke(cli, ["text", "grep", "--root", str(workspace), "match", "docs"])
    ...
    fuzzy_result = runner.invoke(cli, ["file", "search", "--root", str(workspace), "--mode", "fuzzy", "alpah", "docs"])
...
def test_binary_read_write_and_copy(cli, runner, workspace: Path) -> None:
    write_result = runner.invoke(
        cli,
        ["text", "binary-write", "--root", str(workspace), "--data-hex", "a1b2c3", "copy.bin"],
    )
    assert write_result.exit_code == 0, write_result.output

    read_result = runner.invoke(
        cli,
        ["inspect", "bytes", "--root", str(workspace), "--offset", "1", "--length", "2", "copy.bin"],
    )
    assert read_result.exit_code == 0, read_result.output
    read_payload = parse_json(read_result.output)
...
def test_batch_run_dry_run(cli, runner, sample_plan_path: Path, workspace: Path) -> None:
