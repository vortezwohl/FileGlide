"""Commands focused on file lifecycle and file-oriented search."""
...
def create_file_group() -> click.Group:
    ...
    @root_option
    @click.option("--parents/--no-parents", default=False, help="Create missing parent directories.")
    @click.option("--exist-ok", is_flag=True, default=False, help="Allow the file to exist already.")
    ...
    def create_command(runtime, root, parents, exist_ok, target) -> None:
        runtime.executor.execute(
            "file.create",
            [target],
            lambda: runtime.facade.filesystem.create_file(root, target, parents=parents, exist_ok=exist_ok),
            meta={"root": root},
        )
    ...
    @root_option
    @destructive_options
    @click.option("--missing-ok", is_flag=True, default=False, help="Allow deleting a missing file.")
    @click.argument("target")
    @pass_runtime
    ...
    @pass_runtime
    def list_command(runtime, root, include, exclude, max_depth, recursive, start) -> None:
        runtime.executor.execute(
            "file.list",
            [start],
    ...
    @click.argument("start", default=".")
    @pass_runtime
    def search_command(runtime, root, include, exclude, max_depth, recursive, mode, limit, query, start) -> None:
        runtime.executor.execute(
            "file.search",
            [start],
